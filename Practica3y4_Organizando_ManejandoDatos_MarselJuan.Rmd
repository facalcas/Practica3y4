...
reli_tidy<-gather(pew_religion,key="Income",value="Valores",-religion)
reli_tidy$Income<-factor(reli_tidy$Income)
reli_tidy
```
...
```{r}
pivot_longer(raw,names_prefix = "wk",names_to = "week",values_to = "rank",cols = 6:81)
...
```{r}
Tb_unido<-gather(raw,key = "sexo_edad",value = "casos",3:18)
separate(Tb_unido,sexo_edad,c("sexo","edad"),sep = 1)
#Ej3
```
...
```{r}
raw%>%
  pivot_longer(names_to = "dia",values_to = "temp",d1:d31,values_drop_na=T)%>%
  mutate(dia = as.integer(gsub("d", "", dia)), fecha = as.Date(paste(year, month, dia, sep = "-")))%>%
...
head(flights)
head(planes)
#La clave es que comparten una columna que es tailnum, en flights indica cual es el avión que realizó el viaje, y en planes indica información específica del avión en cuestión.
```
...
```{r }
planes.f<-planes%>%
  filter(!str_detect(str_to_lower(manufacturer),".*douglas.*")) %>%
  filter(between(seats,90,120)) %>%
  mutate(manufacturer=ifelse(str_detect(str_to_lower(manufacturer),".*airbus.*"),
                             "AIRBUS COMP",
                             manufacturer))
planes.f
```
...
```{r }
flights <- flights %>%
  mutate(Full_Date=paste(day,month,year,sep="-")) %>%
